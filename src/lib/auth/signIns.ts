 ...
 * `listSignIns` is read back by `loadMembers`, for owners and invited members alike; see
 * `signIns`' own comment in `db/schema.ts` for why it cannot be joined to either table.
 */
...
/**
 * Every address that has ever signed in, for `loadMembers` to look up by email.
 *
 * **null** only when the table could not be read at all — offline, or a database that
 * did not respond — which `loadMembers` treats as "unknown for everyone" rather than
 * failing the whole screen: who may sign in is the question that must stay answerable,
