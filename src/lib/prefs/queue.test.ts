...
describe('prefs write queue', () => {
  ...
  it('keeps only the latest value per target', async () => {
    ...
    queue.enqueueSong('x', { semitones: 1, scrollSpeed: 3, capo: 0, note: '' })
    queue.enqueueSong('x', { semitones: 2, scrollSpeed: 3, capo: 0, note: '' })
    queue.enqueueSong('x', { semitones: 3, scrollSpeed: 3, capo: 0, note: '' })
    assert.equal(queue.size(), 1)
